...
$sqlCupones = $con->prepare("SELECT CUPON.ID, CUPON.TITULO, format(CUPON.PRECIO_OFERTA, 2) AS PRECIO_OFERTA_2, CUPON.FECHA_FIN, CUPON.DESCRIPCION, EMPRESA.NOMBRE_EMPRESA, format(CUPON.PRECIO_REGULAR, 2) AS PRECIO_REGULAR_2 FROM CUPON INNER JOIN EMPRESA ON CUPON.ID_EMPRESA=EMPRESA.ID WHERE CUPON.ID_ESTADO = 1");
$sqlCupones->execute();
